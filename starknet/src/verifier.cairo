...
pub const PROOF_INPUTS_LEN: usize = 3 + TREE_DEPTH;
...
/// - `proof[1]`: stealth tweak
/// - `proof[2]`: eligible leaf index
/// - `proof[3..15]`: Merkle authentication path of fixed depth 12
pub fn verify_claim(claim: ClaimPublicInputs, proof: Span<felt252>) -> bool {
    ...
    let stealth_tweak = *proof.at(1);
    let Some(eligible_index) = (*proof.at(2)).try_into() else {
        return false;
    };

    // Reject zero secrets here so the circuit path stays in the valid proving domain.
    if wallet_secret == 0 || stealth_tweak == 0 {
        return false;
    };
    ...
    let eligible_path = proof.slice(3, TREE_DEPTH);
